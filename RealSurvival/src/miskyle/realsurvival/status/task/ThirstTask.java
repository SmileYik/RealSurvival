...
public class ThirstTask implements Runnable {
  ...
  public void run() {
    PlayerManager.getActivePlayers().forEachValue(PlayerManager.getActivePlayers().mappingCount(), pd -> {
      ...
      RSEntry<Double, Double> values = pd.getThirst().modify(-ConfigManager.getThirstConfig().getDecreaseValue(),
      ...
      for (RSEntry<Double, Double> entry : ConfigManager.getThirstConfig().getEffectData().keySet()) {
        ...
        if (entry.getLeft() > entry.getRight() && values.getLeft() > entry.getLeft()
          ...
          PlayerManager.bar.sendActionBar(p, Msg.tr("messages.thirst." + entry.getLeft() + "-" + entry.getRight()));
        }
