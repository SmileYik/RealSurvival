...
public class EnergyTask implements Runnable {
  ...
  public void run() {
    PlayerManager.getActivePlayers().forEachValue(PlayerManager.getActivePlayers().mappingCount(), pd -> {
      ...
      } else if (ConfigManager.getEnergyConfig().getDecreaseSprinting() != 0 && p.isSprinting()) {
  ...
  public static void sendMessage(Player p, RSEntry<Double, Double> values) {
    for (RSEntry<Double, Double> entry : ConfigManager.getEnergyConfig().getEffectData().keySet()) {
      if (entry.getRight() >= entry.getLeft() && values.getLeft() <= entry.getLeft()
        ...
        PlayerManager.bar.sendActionBar(p, Msg.tr("messages.energy." + entry.getLeft() + "-" + entry.getRight()));
        break;
  ...
  public static void attachEffect(Player p, double value) {
    for (RSEntry<Double, Double> entry : ConfigManager.getEnergyConfig().getEffectData().keySet()) {
      if (value < Math.max(entry.getLeft(), entry.getRight()) && value >= Math.min(entry.getLeft(), entry.getRight())) {
