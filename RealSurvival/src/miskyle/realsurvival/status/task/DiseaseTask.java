...
public class DiseaseTask implements Runnable {
  ...
  public void run() {
    PlayerManager.getActivePlayers().forEachValue(PlayerManager.getActivePlayers().mappingCount(), pd -> {
      ...
      pd.getDisease().getDiseases().forEachValue(pd.getDisease().getDiseases().mappingCount(), disease -> {
        if (disease.recover()) {
          pd.getDisease().removeDisease(disease.getDiseaseName());
          PlayerManager.bar.sendActionBar(p, Msg.tr("messages.disease", disease.getDiseaseName()));
        } else {
          ConfigManager.getDiseaseConfig().getDiseaseEffect(disease.getDiseaseName()).forEach(effect -> {
