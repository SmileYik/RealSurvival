...
public class RandomDayManager {
  ...
  public static void init() {
    ...
    RandomDayConfig.getWorldDatas().forEach((k, v) -> {
      ...
      Bukkit.getScheduler().runTaskTimerAsynchronously(MCPT.plugin, task, 0L, RandomDayConfig.getTick());
      tasks.put(k, task);
    });
  }
    ...
    for (int x = b.getBlockX() - 2; x <= b.getBlockX() + 2; x++)
      for (int y = b.getBlockY() - 2; y <= b.getBlockY() + 2; y++)
        for (int z = b.getBlockZ() - 2; z <= b.getBlockZ() + 2; z++) {
          ...
          if (!block.isEmpty()) {
            double blockT = ConfigManager.getTemperatureConfig().getBlockTemperature(block.getType().name());
