...
public class RSCommand implements CommandExecutor, TabExecutor {
  ...
  public boolean onCommand(CommandSender sender, Command command, String topCmd, String[] args) {
    ...
    for (Method method : methods) {
      ...
      if (args.length < cmd.args().length)
        continue;
      if (!CommandManager.compareSubCommand(args, cmd.subCmd()))
        continue;
      if (!cmd.permission().isEmpty() && !sender.hasPermission(cmd.permission()))
        continue;
      if (!cmd.unlimitedLength() && args.length != cmd.args().length)
        continue;
  ...
  public List<String> onTabComplete(CommandSender commandSender, Command command, String a, String[] args) {
    ...
    return Arrays.stream(subCmds.get(args.length - 1)).filter(s -> s.startsWith(args[args.length - 1]))
